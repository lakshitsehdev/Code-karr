...
import { Input, Button, Modal, FormGroup, Row, Col, Label } from "reactstrap";
// core components
...
function EditSection({ open, isOpen, submit, section }) {
  const { setFileListOptions, setSelectOptions, setErrorOptions } =
    useAlertState();
  ...
  const [files, setFiles] = React.useState(
    section.type === "coding" ? section.coding?.files || [] : []
  );
  ...
  React.useEffect(() => {
    if (type !== "info" && layout === 3) setLayout(0);
  }, [type, layout]);
  ...
  const finish = async () => {
    ...
    switch (type) {
      ...
      case "coding":
        ...
        completed.coding.checks = checks.map((c) => ({
          ...c,
          desc: undefined,
        }));
    ...
    if (section.code) completed.code = section.code;
  ...
  const newCase = () => {
    setChecks([
      ...checks,
      {
        ...
        stdout: "",
        hint: "",
        desc: "Test cases for programming challenges:",
      },
    ]);
  };
  const newCode = () => {
    setChecks([
      ...checks,
      {
        ...
        multiline: true,
        fail: false,
        desc: "Regex to verify the code and output:",
      },
    ]);
  };
      ...
      <Modal toggle={() => open(false)} isOpen={isOpen} className="modal-xl">
        <div className="modal-header">
          <h5 className="modal-title">
            ...
            {section.code && (
              <p className="mb-0">
                <small>Section ID: {section.code}</small>
              </p>
            )}
          </h5>
          <button
        ...
        <div className="modal-body">
            ...
            <FormGroup>
              ...
              <div className="d-flex">
                <div>
                  <Button
                    className="p-2"
                    color={layout === 0 ? "info" : "secondary"}
                    onClick={() => setLayout(0)}
                  >
                    <img
                      alt="50 / 50"
                      src={require("assets/img/section/layout0.png")}
                      style={{ height: "4rem" }}
                    />
                  </Button>
                </div>
                <div className="ml-3">
                  <Button
                    className="p-2"
                    color={layout === 1 ? "info" : "secondary"}
                    onClick={() => setLayout(1)}
                  >
                    <img
                      alt="25 / 75"
                      src={require("assets/img/section/layout1.png")}
                      style={{ height: "4rem" }}
                    />
                  </Button>
                </div>
                <div className="ml-3">
                  <Button
                    className="p-2"
                    color={layout === 2 ? "info" : "secondary"}
                    onClick={() => setLayout(2)}
                  >
                    <img
                      alt="75 / 25"
                      src={require("assets/img/section/layout2.png")}
                      style={{ height: "4rem" }}
                    />
                  </Button>
                </div>
                {type === "info" && (
                  <div className="ml-3">
                    <Button
                      className="p-2"
                      color={layout === 3 ? "info" : "secondary"}
                      onClick={() => setLayout(3)}
                    >
                      <img
                        alt="75 / 25"
                        src={require("assets/img/section/layout3.png")}
                        style={{ height: "4rem" }}
                      />
                    </Button>
                  </div>
          ...
          {type === "coding" && (
            <FormGroup>
                ...
                <div key={i} className="mt-3">
                  ...
                  <Row>
                    {Object.keys(check)
                      .filter(
                        (p) =>
                          !["desc", "multiline", "fail", "hint"].includes(p) &&
                          p
                      )
                      .map((prop, j) => (
                        <Col key={j}>
                          ...
                          <Input
                            ...
                            value={check[prop]}
                            onChange={(e) =>
                              updateCheck(i, prop, e.target.value)
                            }
                          />
                        </Col>
                      ))}
                  ...
                  <Row>
                    {Object.keys(check)
                      .filter((p) => ["multiline", "fail"].includes(p) && p)
                      .map((prop, j) => (
                        <Col key={j} className="ml-3">
                          <Label check>
                            <Input
                              ...
                              checked={check[prop]}
                              onChange={(e) =>
                                updateCheck(i, prop, e.target.checked)
                              }
                            />{" "}
                            {prop} #{i + 1}
                          </Label>
              ...
              <br />
              <Button color="black" type="button" size="sm" onClick={newCase}>
                + Test Case
              </Button>
              <Button color="primary" type="button" size="sm" onClick={newCode}>
                + Code Check
              </Button>
              {checks.length > 0 && (
                <Button
          ...
          {type === "quiz" && (
            <FormGroup>
                      ...
                      <FormGroup check>
                        <Label check>
                          <Input
                            ...
                            onChange={(e) =>
                              updateAns(i, "correct", e.target.checked)
                            }
                          />
              ...
              <Button color="black" type="button" size="sm" onClick={newAns}>
                +
              </Button>
              <Button color="danger" type="button" size="sm" onClick={delAns}>
                -
              </Button>
            </FormGroup>
          )}
            ...
            <Row>
              <Col>
                ...
                {!autopass && (
                  <div>
                    Run the JS snippet{" "}
                    <code>window.parent.postMessage("finish", "*");</code> to
                    finish the section.
                  </div>
                )}
              </Col>
          ...
          <Button
            ...
            size="sm"
            onClick={() =>
              setFileListOptions({ title: "File Manager", key: Math.random() })
            }
          >
            File Manager
          </Button>
        ...
        <div className="modal-footer">
          <Button color="danger" type="button" onClick={() => open(false)}>
            Close
          </Button>
          <Button color="black" type="button" onClick={finish}>
            Save
          </Button>
        </div>
      </Modal>
