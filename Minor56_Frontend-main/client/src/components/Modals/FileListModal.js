...
import { Button, Modal, Row, Col, Container, Progress } from "reactstrap";
// core components
...
function FileListModal({
  ...
  const update = React.useCallback(() => {
    ...
    let subfolders = response
      .map((s) => s.folder)
      .filter(
        (f) =>
          f !== cwd &&
          f.startsWith(cwd) &&
          f.split("/").length === base.split("/").length + 1
      );
  ...
  const copyFile = (i) => {
    navigator.clipboard.writeText(
      process.env.REACT_APP_API_URL + "/file/" + files[i].code
    );
      ...
      submit: (status) => {
        if (status) {
          fetch(process.env.REACT_APP_API_URL + "/file/delete", {
            ...
            .then((r) => r.json())
            .then((json) => {
              if (json.success) setMessageOptions({ body: json.response });
              else setErrorOptions({ body: json.response });
            ...
            <Row className="file-list-container">
              {cwd !== "/" && (
                ...
                >
                  ...
                  <Col className="file-list-desc">..</Col>
                </Row>
              )}
              {folders &&
                ...
                <Button
                  ...
                  onClick={() => {
                    submitFolder(
                      JSON.parse(
                        JSON.stringify(
                          response.filter((s) => s.folder.startsWith(cwd))
                        )
                      ),
                      cwd
                    );
            ...
            <Row>
              <div className="progress-container progress-danger w-100">
                <span className="progress-badge">
                  Storage Remaining ({parseInt(128 * (1 - space / 100))} /
                  128MB)
                </span>
