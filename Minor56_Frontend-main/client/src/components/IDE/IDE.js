...
import { Col, Spinner, Button, UncontrolledTooltip, Badge } from "reactstrap";
...
function IDE({
  ...
  const connectWS = React.useCallback(() => {
    ...
    let socket = new WebSocket(
      process.env.REACT_APP_API_URL.replace("https", "wss").replace(
        "http",
        "ws"
      ) + "/ws"
    );
  ...
  React.useEffect(() => {
    ...
    ws.onmessage = function (event) {
      ...
      if (data.type === "stdout") {
        if (!data.msg.startsWith("[Task]")) setStatus("connected");
        setActive((prev) => ({
          ...prev,
          output: prev.output.concat([{ type: "stdout", content: data.msg }]),
        }));
      } else if (data.type === "stderr") {
        if (!data.msg.startsWith("[Task]")) setStatus("connected");
        setActive((prev) => ({
          ...prev,
          output: prev.output.concat([{ type: "stderr", content: data.msg }]),
        }));
        ...
        if (data.meta === "error") {
          ...
          setInputOptions({
            value: window.location.origin + "/ide?collab=" + data.msg,
            title: "Collab URL",
            body: "Send this URL to someone you want to share your code with.",
          });
        } else if (data.meta === "update") {
          if (data.msg) setActive(data.msg);
  ...
  React.useEffect(() => {
    if (!section || !collab || status === "disconnected") return;
    ws.send(
      JSON.stringify({
        type: "collab",
        meta: "leave",
      ...
      .then(async (json) => {
        if (json.success) {
          ...
          const loadFiles = async () => {
            for (let location of files) {
              for (let i = 0; i < location.files.length; i++) {
                if (!location.folder.endsWith("/")) location.folder += "/";
                location.files[i].content = await (
                  await fetch(
                    process.env.REACT_APP_API_URL +
                      "/file/" +
                      location.files[i].code
                  )
                ).text();
              }
            }
          ...
          if (!useFileStorage && storageKey && storage.load(storageKey)) {
            ...
            setActive({
              ...saved,
              lang: langs.find((l) => l.lang === saved.lang.lang),
              output: [],
              input: "",
              loaded: true,
            });
            ...
            if (lang.template[0].files[0]) {
              open = [
                {
                  filename: lang.template[0].files[0].filename,
                  folder: lang.template[0].folder,
                },
              ];
            }
            let start = null;
  ...
  React.useEffect(() => {
    function handleResize() {
      if (navbarRef.current && codeBottomRef.current && codeTopRef.current) {
        ...
        let prevHeight =
          document.getElementsByClassName("react-codemirror2")[0].style.height;
      ...
      submit: (status) => {
        if (status) {
          setActive((prev) => ({
            ...
            open: [
              {
                filename: active.lang.template[0].files[0].filename,
                folder: active.lang.template[0].folder,
              },
            ],
            loaded: true,
            sideFolder: "",
  ...
  return (
    <>
      <Col
        sm="12"
        className={size === "normal" ? "col-md-4 ide" : "col-md-6 ide"}
      >
        ...
        {active.file && active.lang ? (
          <CodeMirror
            ...
            onBeforeChange={(editor, data, value) => {
              ...
              let location = active.files.findIndex(
                (f) => f.folder === active.folder
              );
              if (location !== -1) {
                let index = active.files[location].files.findIndex(
                  (f) => f.filename === active.file.filename
                );
        ...
        <div className="ide-bottom" ref={codeBottomRef}>
          {status === "disconnected" ? (
            ...
            <div>
              ...
              <UncontrolledTooltip placement="top" target="ide-run">
                Run
              </UncontrolledTooltip>
              ...
              <UncontrolledTooltip placement="top" target="ide-reset">
                Reset
              </UncontrolledTooltip>
              ...
              <UncontrolledTooltip placement="top" target="ide-input">
                Input
              </UncontrolledTooltip>
              ...
              {languages && languages.length > 1 && (
                <>
                  ...
                  <UncontrolledTooltip placement="top" target="ide-language">
                    {active.lang ? active.lang.name : "Loading..."}
                  </UncontrolledTooltip>
                </>
              )}
              ...
              {checks && (
                <>
                  <Button
                    disabled={status !== "connected"}
                    id="ide-check"
                    className="m-0 ml-2 float-right"
                    color="danger"
                    type="button"
                    size="sm"
                    onClick={check}
                  ...
                  <UncontrolledTooltip placement="top" target="ide-check">
                    Check
                  </UncontrolledTooltip>
                </>
              )}
              {useFileStorage && !collabCode && base && (
                <>
                  <Button
                    ...
                    type="button"
                    size="sm"
                    onClick={() => onSave(clone(active.files))}
                  >
                    <i className="fas fa-save"></i>
                  </Button>
                  <UncontrolledTooltip placement="top" target="ide-save">
                    Save
                  </UncontrolledTooltip>
                </>
              )}
              ...
              <UncontrolledTooltip placement="top" target="ide-collab">
                Collab
              </UncontrolledTooltip>
            </div>
          )}
      ...
      <Col
        ...
        className={
          size === "normal"
            ? "col-md-4 room-output p-0"
            : "col-md-6 room-output p-0"
        }
      >
        <div className="ide-top">
          {active.lang && active.lang.name ? active.lang.name : "Loading..."}
          {status === "connected" && (
            <Badge color="success" className="m-0 mr-2 ide-badge float-right">
              Connected<i className="fas fa-check ml-2"></i>
            </Badge>
          )}
          {status === "pending" && (
            <Badge color="warning" className="m-0 mr-2 ide-badge float-right">
              Sending
              <Spinner size="sm" type="grow" className="ml-2 ide-grow" />
            </Badge>
          )}
          {status === "disconnected" && (
            <Badge color="danger" className="m-0 mr-2 ide-badge float-right">
              Disconnected<i className="fas fa-times ml-2"></i>
            </Badge>
          )}
        </div>
        <div className="p-3 room-terminal">
          {active.output &&
            active.output.map((message, i) =>
              message.type === "stdout" ? (
                <div key={i}>{message.content}</div>
              ) : (
                <div key={i} className="room-output-error">
                  {message.content}
                </div>
              )
            )}
