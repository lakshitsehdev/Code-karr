...
function CreatePage() {
  ...
  const editSection = (title) => {
    let section = sectionsRef.current.find(
  ...
  const deleteSection = (title) => {
    setSections(
      [...sectionsRef.current].filter((check) => check.title !== title)
    );
  };
  ...
  const createSection = (title) => {
    if (sectionsRef.current.find((section) => section.title === title)) {
      return setErrorOptions({
        body: "A section already exists with that title.",
      });
    }
    if (!title) {
  ...
  const finishSection = (modified) => {
    let index = sectionsRef.current.findIndex(
      (section) => section.title === sectionRef.current.title
    );
  ...
  const saveRoom = () => {
    ...
    fetch(
      process.env.REACT_APP_API_URL +
        (isEditing ? "/room/edit" : "/room/create"),
      {
        method: "POST",
        headers: {
        ...
        .then((json) => {
          if (json.success) {
            if (json.response.author !== user) {
              return setErrorOptions({
                body: "You are not this room's creator!",
                submit: () => {
                  history.push("/home");
                },
              });
            ...
            for (let i = 0; i < json.response.sections.length; i++) {
              ...
              if (
                json.response.sections[i].checks &&
                json.response.sections[i].checks.length === 0
              )
                delete json.response.sections[i].checks;
              if (
                json.response.sections[i].answers &&
                json.response.sections[i].answers.length === 0
              )
            ...
            setErrorOptions({
              body: "No room was found with that code.",
              submit: () => {
                history.push("/home");
              },
            });
    ...
    <>
      ...
      <div className="wrapper">
        <EditSection
          ...
          submit={finishSection}
          key={sectionRef.current.title}
        />
        <ExportModal
          open={setExportModal}
          isOpen={exportModal}
          data={JSON.stringify(
            { title, desc, sections: sectionsRef.current, public: isPublic },
            null,
            " ".repeat(4)
          )}
        />
