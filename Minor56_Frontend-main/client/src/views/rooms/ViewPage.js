...
function ViewPage() {
            ...
            .then((json) => {
              if (json.success) {
                complete(
                  ...
                  section.type === "info" ||
                    (section.type === "coding" &&
                      section.coding.checks.length === 0) ||
  ...
  if (!section) {
    return (
      ...
      >
        <Spinner color="black" style={{ width: "8rem", height: "8rem" }} />
      </div>
    );
  }
  ...
  const RoomButtons = (num) => (
    <div className="room-buttons">
      {loaded && num > 0 && (
        <Button color="danger" type="button" onClick={() => setNum(num - 1)}>
          <i className="fas fa-arrow-left"></i> Prev
        </Button>
      )}
      {loaded && num <= room.sections.length - 2 && section.completed && (
        <Button color="black" type="button" onClick={() => setNum(num + 1)}>
        ...
        <Button
          ...
          onClick={() =>
            setMessageOptions({
              ...
              body: "You have completed the room!",
              submit: () => history.push("/home"),
            })
          }
        >
        ...
        {section.type === "info" && (
          <>
            <div
              className={
                "room-col-text " +
                ["c-half-l", "c-small-l", "c-large-l", "vw-100"][section.layout]
              }
            >
              <div className="mt-3 room-type">{section.type}</div>
              <h6 className="title room-text">
                {room.title} ({num + 1}/{room.sections.length})
              </h6>
              <h5 className="title room-text">
                {section.title}{" "}
                {section.completed && (
            ...
            <div
              className={
                ["c-half-r", "c-large-r", "c-small-r", "d-none"][section.layout]
              }
            >
        ...
        {section.type === "quiz" && (
          <>
            <div
              className={
                "room-col-text " +
                ["c-half-l", "c-small-l", "c-large-l"][section.layout]
              }
            >
              <div className="mt-3 room-type">{section.type}</div>
              <h6 className="title room-text">
            ...
            <div
              className={
                "room-col-text pl-3 " +
                ["c-half-r", "c-large-r", "c-small-r"][section.layout]
              }
            >
                  ...
                  <div key={i}>
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        if (section.quiz.all) {
                          setAnswers((prevAns) =>
                            console.log(prevAns) || checked
                              ? [...new Set(prevAns.concat(answer.choice))]
                              : answers.filter((a) => a !== answer.choice)
                          );
                        } else {
                    ...
                    />
                    <h5 className="ml-6">
                      <label htmlFor={`answer_${i}`}>{answer.choice}</label>
                    </h5>
                  </div>
                ))}
              </Form>
